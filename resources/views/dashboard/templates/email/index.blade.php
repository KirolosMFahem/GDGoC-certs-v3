                                ...
                                @forelse($userTemplates as $template)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            <a href="{{ route('dashboard.templates.email.edit', $template) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            
                                            @if($template->original_template_id)
                                                <form action="{{ route('dashboard.templates.email.reset', $template) }}" method="POST" class="inline">
                                                    @csrf
                                            ...
                                            <form action="{{ route('dashboard.templates.email.destroy', $template) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">
                                            No templates found. <a href="{{ route('dashboard.templates.email.create') }}" class="text-blue-600 hover:text-blue-800">Create one</a> or clone from global templates below.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                                ...
                                @forelse($globalTemplates as $template)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <form action="{{ route('dashboard.templates.email.clone', $template) }}" method="POST" class="inline">
                                                @csrf
